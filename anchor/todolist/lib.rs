...
mod solana_todo {
    ...
    pub fn mark_todo(ctx:Context<MarkTodo>,todo_idx:u8)->Result<()>{
        ...
        let todo_account = &mut ctx.accounts.todo_account;
        require!(!todo_account.marked, "CustomError");
        todo_account.marked  = true;
        Ok(());
    }

      pub fn remove_todo(ctx:Context<MarkTodo>,todo_idx:u8)->Result<()>{
        //let user_profile = ctx.accounts.user_profile;
        let user_profile = &mut ctx.accounts.user_profile;
...
pub struct AddTodo<'info>{
    ...
    pub user_profile:Box<Account<'info,UserProfile>>,
    #[account(mut,seeds=[TODO_TAG,authority.key.as_ref(),&[user_profile.last_todo].as_ref()],bump
